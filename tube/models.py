...
class NotifyTarget(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        send_mail(
            ...
            'tubeintegrity.com よりお知らせがあります。通知欄をご確認ください。',
...
class Activity(models.Model):
    ...
    """
    saveした時に加点、deleteした時に減点する?  (悪いねは集計の計算でマイナス処理をするので、他と同様で問題ない。)
    ↑編集処理はどうする？←.save()で編集するたびの加点を防ぐため、事前にあるかないかを確認した上で処理するのは？saveする際、対象のオブジェクトからIDを参照できると思われる。
    """

"""
#TODO:再考の余地あり。ランキング系のモデルクラスのrankはorder_byに任せるべきでは？ランキングのタイを実装させるだけでとてつもなく複雑な処理系になってしまう。
class DailyRank(models.Model):
    ...
    date        = models.DateField(verbose_name="実行日",default=datetime.date.today)
    target      = models.ForeignKey(Video,verbose_name="対象動画",on_delete=models.CASCADE)
    category    = models.ForeignKey(VideoCategory,verbose_name="集計時点でのランキング部門(nullは総合部門)",on_delete=models.PROTECT,null=True,blank=True)
